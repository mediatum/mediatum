...
#url to ping google the sitemapindex location
PING_URL_ENCODED = 'http://www.google.com/webmasters/tools/ping?sitemap=http%3A%2F%2Fmediatum.ub.tum.de%2Fsitemapindex.xml'
...
class Sitemap:
    ...
    def create_sitemap(self, nodes, p_num):
        ...
        @param p_num: Priority number for the sitemap entry in the format #.# from 0.0 to 1.0
        """
        if os.path.isfile(self.path):
            ...
            if not nodes:
                ...
                for i in nodes:
                    ...
                    loc.text = ''.join([self.host, '/node?id=', i[0]])
                    lastmod = etree.SubElement(url, 'lastmod')
                    #If lastmod field was empty -> now will be +02:00 -> prepend current time
                    if i[1] == '+02:00':
...
class SitemapIndex:
    ...
    def create_sitemap_index(self, sitemaps, cur_time):
        ...
        @param cur_time: the current time in datetime format
        """
        if os.path.isfile(self.path):
    ...
    for i in node_dict.keys():
        ...
        if len(node_dict[i]) > 50000:
            ...
            for j in range(partitions):
                sitemap = Sitemap(base_dir, ''.join([str(i), str(j), '.xml']), hostname)
                sitemaps.append(sitemap.name)
                sitemap.create_sitemap(node_dict[i][j*50000:(j+1)*50000], priority_level)
        else:
            sitemap = Sitemap(base_dir, ''.join([i, '.xml']), hostname)
            sitemaps.append(sitemap.name)
            sitemap.create_sitemap(node_dict[i], priority_level)

    siteindex = SitemapIndex(base_dir, 'sitemapindex.xml', hostname)
...
def main():
    """
    Takes in the command line args [create] or [clean] and generates the sitemaps, sitemapindex and then send a ping to google
    """
    if len(sys.argv) > 2 or 'create' not in sys.argv and 'clean' not in sys.argv:
        raise TypeError('sitemaps.py takes in only one of the following arguments: [create] [clean]')
    elif sys.argv[1] == 'create':
        clean()
        create()
        response = urllib2.urlopen(PING_URL_ENCODED)
        if response.getcode() == 200:
            logging.getLogger('everything').info('Successful ping of sitemapindex.xml to Google; Response Code: %i' % response.getcode())
        else:
            logging.getLogger('everything').info('Unsuccessful ping of sitemapindex.xml to Google; Response Code: %i' % response.getcode())
