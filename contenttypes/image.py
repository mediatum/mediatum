...
class Image(default.Default):
    ...
    def event_files_changed(self):
        ...
        if "image" in self.type:
            ...
            for f in self.getFiles():
                if f.getType() == 'original':
                    tags_in_upload = IPTC.get_iptc_values(f.retrieveFile())

                    if tags_in_upload:
                        for key in tags_in_upload.keys():
                            if key not in IPTC.get_wanted_iptc_tags():
                                continue
                            if tags_in_upload[key] != '':
                                self.set('iptc_{}'.format(key.replace(' ', '_')),
                                         utf8_decode_escape(str(tags_in_upload[key])))
    ...
    def event_metadata_changed(self):
        """ Handles metadata content if changed.
            Creates a 'new' original.
            Uploaded old original under 'upload'.
        ...
        for f in self.getFiles():
            ...
            if f.getType() == 'original':
                ...
                tag_object = IPTC.get_iptc_object(original_path)
                write_path = '{}/{}_WRITE{}'.format(os.path.dirname(original_path),
                                                    os.path.splitext(os.path.basename(original_path))[0],
                                                    os.path.splitext(os.path.basename(original_path))[1])
        if not upload_file:
            if original_file:
                upload_file = importFileRandom(original_file.retrieveFile())
                upload_file.type = 'upload'
                upload_file.mimetype = original_file.mimetype
                upload_file.node = self
                self.addFile(upload_file)

        if tag_object and write_path:
            for field in getMetaType(self.getSchema()).getMetaFields():
                if field.get('type') == "meta" and len(field.getValueList()) > 1:
                    ...
                    if value != '':
                        tag_object.setIPTCTag(field.getValueList()[0], value)
            try:
                tag_object.writeFile(write_path)
                os.remove(original_path)
                os.rename(write_path, original_path)
            except IOError:
                logger.error('Try to write, rename file: {} and to remove file: {}.').format(write_path, original_path)