...
def getHelp(req):
    ...
    indexchars = list(set([i[0].upper() for i in index[language].keys()]))
    indexchars.sort()
...
class HelpPdf:
    ...
    def getStyle(self, page):
        ...
        if page==1: # first page
            ...
            frames.append(Frame(1*cm, 2*cm, 19*cm, 26.5*cm,leftPadding=0, rightPadding=0, id='normal', showBoundary=0)) # content page>1
        return frames
    ...
    def build(self, style=1):
        ...
        template.canv.setTitle("%s \n'%s' - %s: %s" %(translation.t(self.language, "edit_stats_header"), 'sdfsdfsdf',translation.t(self.language, "edit_stats_period_header"), '2003'))
