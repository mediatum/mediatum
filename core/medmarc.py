...
from schema import mapping as _schema_mapping
from pymarc import constants as _pymarc_constants
...
def find_marc_mapping(schema_name):
    ...
    """
    marc_mapping = _core.db.query(_schema_mapping.Mapping).filter_by(name='marc21_' + schema_name).scalar()
    if marc_mapping is None:  # FIXME: or not marc_mapping.isActive():
        return None

    mapping_fields = [(tuple(mask_item.name.split('$', 1)),
                       _utils_utils.Template(mask_item.getDescription()))
...
class UnicodeRecord(pymarc.Record):
    ...
    def as_marc(self):
        ...
        directory += _pymarc_constants.END_OF_FIELD.encode(encoding)

        # field data ends with an end of record
        fields += _pymarc_constants.END_OF_RECORD.encode(encoding)

        # the base address where the directory ends and the field data begins
        base_address = _pymarc_constants.LEADER_LEN + len(directory)
