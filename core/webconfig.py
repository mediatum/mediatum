...
def startWebServer():
    ...
    # === public area ===
    file = context.addFile("frontend/streams.py")
    ...
    file = context.addFile("frontend/treeframe.py")
    file.addHandler("treeframe").addPattern("/treeframe")
    file = context.addFile("frontend/main.py")
    ...
    file = context.addFile("frontend/popups.py")
    ...
    file = context.addFile("frontend/login.py")
    ...
    file = context.addFile("frontend/usersettings.py")
    file.addHandler("user_settings").addPattern("/user")

    # === admin area ===
    context = athana.addContext("/admin", ".")
    file = context.addFile("admin/main.py")
    ...
    file = context.addFile("edit/edit.py")
    ...
    file = context.addFile("edit/edit_upload.py")
    file.addHandler("upload_new").addPattern("/upload_new")
    file = context.addFile("edit/edit_license.py")
    file.addHandler("objlist").addPattern("/objlist")

    # === OAI ===
    context = athana.addContext("/oai/", ".")
    file = context.addFile("oai.py")
    file.addHandler("oaiRequest").addPattern(".*")

    # === static files ===
    athana.addFileStore("/module/", "mod/FCKeditor/files.zip")
    athana.addFileStore("/css/", "template/tum/css/")
    athana.addFileStore("/img/", ["template/tum/img/", "img"])
    athana.addFileStore("/js/", ["template/tum/js/", "js"])
