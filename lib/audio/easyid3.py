...
#
# $Id: easyid3.py,v 1.1 2010/05/18 15:12:56 seiferta Exp $
...
from lib.audio import Metadata, FileType
    ...
    def RegisterTextKey(cls, key, frameid):
        ...
        def setter(id3, key, value):
            try:
                ...
                id3.add(id3.Frames[frameid](encoding=3, text=value))
            else:
                frame.encoding = 3
                frame.text = value
        ...
        def setter(id3, key, value):
            try:
                ...
                for v in value:
                    if max(v) > u'\x7f':
                        enc = 3
                id3.add(id3.TXXX(encoding=enc, text=value, desc=desc))
            else:
                frame.text = value
...
def genre_set(id3, key, value):
    try:
        ...
        id3.add(id3.TCON(encoding=3, text=value))
    else:
        frame.encoding = 3
        frame.genres = value
...
def performer_set(id3, key, value):
    ...
    try:
        ...
        mcl = id3.TMCL(encoding=3, people=[])
        id3.add(mcl)
    mcl.encoding = 3
...
def gain_set(id3, key, value):
    ...
    try:
        ...
        frame = id3.RVA2(desc=key[11:-5], gain=0, peak=0, channel=1)
        id3.add(frame)
    frame.gain = gain
