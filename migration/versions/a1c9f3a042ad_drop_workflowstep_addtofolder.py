...
def upgrade():
    for node in _core.db.query(_core.Node).filter(_core.Node.type == 'workflowstep_addtofolder').prefetch_attrs():
...
def downgrade():
    ...
    for node in _core.db.query(_core.Node).filter(_core.Node.type == 'workflowstep_textpage').\
            filter(_core.Node.attrs[alembic_revision] == '"1"').prefetch_attrs():
