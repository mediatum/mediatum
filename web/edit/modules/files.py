...
def _finish_change(node, change_file, user, uploadfile, req):
    ...
    if change_file == "addthumb": # create new thumbanil from uploaded file
        ...
        node.files.append(File(thumbname, "thumbnail", "image/jpeg"))
...
def getContent(req, ids):
    ...
    logg.debug("%s|web.edit.modules.files.getContend|req.path=%s|req.mediatum_contextfree_path=%s|req.params=%s|ids=%s",
