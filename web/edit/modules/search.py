...
def search_form(req, id, message=None):
    ...
    if otype:
        for field in otype.getMetaFields():
            if field.Searchfield() and field.getFieldtype() != "date":
                ...
                c = Context(field, value, width=640, name=field.getName(), collection=node,
                            language=lang(req), user=getUserFromRequest(req))
    ...
    try:
        ...
        indexdate = None
    ret += req.getTAL("web/edit/modules/search.html",
                      {"node": node,
                       "occur": occur,
                       "mtypelist": mtypelist,
                       "objtype": objtype,
