...
class StatsAccessPDF:
    ...
    def getStyle(self, page):
        ...
        if page == 1:  # first page
            ...
            # main header pages >1
            frames.append(Frame(1 * cm, 27.5 * cm, 19 * cm, 1 * cm, leftPadding=0, rightPadding=0, id='normal', showBoundary=0))
            # countries
            ...
            for i in range(0, 45):
                if i < len(d):
                    ...
                    items.append(Frame(9 * cm,
                                       ...
                                       id='normal',
                                       showBoundary=0))  # bar
                    items.append(Frame(9 * cm + (d[i][1] * 280 / max) + 5, (22.9 - (i * 0.5)) * cm, 40,
                                       25, leftPadding=0, rightPadding=0, id='normal', showBoundary=0))  # number
    ...
    def getStatCountry(self, type):
        ...
        if type == "frames":
            # header country
            items.append(Frame(1 * cm, 26.0 * cm, 19 * cm, 1 * cm, leftPadding=0, rightPadding=0, id='normal', showBoundary=0))
            i = 0
            for v in sorted([(len(d[k]['items']), k) for k in filter(lambda x:x != 0, d.keys())], reverse=True):
                if v[0] != 0:
                    ...
                    items.append(Frame(4 * cm,
                                       ...
                                       id='normal',
                                       showBoundary=0))  # bar
                    items.append(Frame(4 * cm + (v[0] * 400 / max) + 5, (25.1 - (i * 0.5)) * cm, 40, 25,
                                       leftPadding=0, rightPadding=0, id='normal', showBoundary=0))  # number
                    i += 1
    ...
    def getStatDate(self, type):
        ...
        if type == "frames":
            ...
            for k in d:
                if k == 0:
                    continue
                items.append(Frame(1 * cm + (k - 1) * 17 + 9 + x,
                                   18.9 * cm,
                                   5,
                ...
                items.append(Frame(1 * cm + (k - 1) * 17 + 14 + x,
                                   18.9 * cm,
                                   5,
                                   len(d[k]["different"]) * (6 * cm) / max["max_p"] + 16,
                                   leftPadding=0,
                                   rightPadding=0,
                                   id='normal',
                                   showBoundary=0))  # col 2
                items.append(Frame(1 * cm + (k - 1) * 17 + 19 + x,
                                   18.9 * cm,
                                   5,
            ...
            items.append(Frame(1 * cm, 17.8 * cm, 19 * cm, 1.5 * cm, leftPadding=4, rightPadding=0, id='normal', showBoundary=0))
            # table with values
        ...
        if type == "frames":
            ...
            for k in d:
                ...
                items.append(Frame(l + (k - 1) * 17 + 5,
                                   ...
                                   id='normal',
                                   showBoundary=0))  # col 2
                items.append(Frame(l + (k - 1) * 17 + 10,
                                   b,
                                   16,
    ...
    def getStatTime(self, type):
        ...
        if type == "frames":
            # header time
            items.append(Frame(1 * cm, 19.3 * cm, 19 * cm, 1 * cm, leftPadding=0, rightPadding=0, id='normal', showBoundary=0))

            for k in d:
                if k == 0:
                    continue
                items.append(Frame(1.67 * cm + (k - 1) * 21,
                                   14.8 * cm,
                                   6,
                                   (len(d[k]["visitors"]) * h / max["max_u"]) + 16,
                                   leftPadding=0,
                ...
                items.append(Frame(1.67 * cm + (k - 1) * 21 + 6,
                                   14.8 * cm,
                                   6,
                                   (len(d[k]["different"]) * h / max["max_p"]) + 16,
                                   leftPadding=0,
                                   ...
                                   id='normal',
                                   showBoundary=0))  # row 1
                items.append(Frame(1.67 * cm + (k - 1) * 21 + 12,
                                   14.8 * cm,
                                   6,
                                   (len(d[k]["items"]) * h / max["max"]) + 16,
                                   leftPadding=0,
