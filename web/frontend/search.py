        ...
        for field in q(SearchMaskItem).get(field_id_or_name).children:
            ...
            if date_from == "0000-00-00T00:00:00" and date_to != date_from:
                ...
                q_user.append(u'({} {} "{}" {} "{}")'.format(
                        field.name,
                        _core_translation.translate_in_request("search_between", req),
                        from_value,
                        _core_translation.translate_in_request("search_and", req),
                        to_value,
                       ))

        yield (
                u"({})".format(" or ".join(q_str)),
                u" {} ".format(_core_translation.translate_in_request("search_or", req)).join(q_user),
               )
...
def extended_search(req, paths):
    ...
    readable_query = " {} ".format(_core_translation.translate_in_request("search_and", req)).join(readable_query)
