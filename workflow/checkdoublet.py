...
def getAttributeNamesForMask(node, language, maskname="shortview"):
    ...
    if mask:
        ordered_fields = [(f.orderpos, f) for f in mask.getMaskFields()]
        ordered_fields.sort()
...
class WorkflowStep_CheckDoublet(WorkflowStep):
    ...
    def show_workflow_node(self, node, req):
        ...
        step_children_list = [(wf_step, wf_step_children, [x.id for x in wf_step_children]) for (wf_step, wf_step_children) in step_children_list]
        ...
        dict_labels = dict([(attr_name, getLabelForAttributename(schema, attr_name, masklist_for_labels)) for attr_name in attribute_names + additional_attributes])   
        
        additional_attributes_decorated = [(dict_labels[x], x) for x in additional_attributes]    
        additional_attributes_decorated.sort()
        ...
        if len(doublets)==1:
            if DEBUG: logging.getLogger('workflows').info('checkdoublet: node %s: no doublet found' % node.id)
