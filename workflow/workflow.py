...
class Workflow(Node):
    ...
    def show_node_big(self, req, *args): 
        ...
        if self.children.filter_write_access().first() is None:
            return '<i>' + _core_translation.translate(_core_translation.set_language(req.accept_languages), "permission_denied") + '</i>'
        return _tal.processTAL(
                dict(
                    workflow=self,
                    search=req.values.get("workflow_search", ""),
    ...
    def show_node_image(node):
        return '<img border="0" src="/static/img/webtree/directory.png">'
...
class WorkflowStep(Node):
    ...
    def show_workflow_notexist(self, node, req, template="workflow/workflow.html", macro="workflow_node"):
        ...
        if step:
            ...
            return '<i>{}</i>'.format(_core_translation.translate(_core_translation.set_language(req.accept_languages), "permission_denied"))
